...
namespace GeekTrustTests
{
    class BaseTest
    {
        ...
        public void PositiveTest(int fileNum)
        {
            ...
            inputPath = GetInputPath(fileNum);
        ...
        public void NegativeTests(int fileNum)
        {
            // Creates the Input File path
            inputPath = GetInputPath(fileNum);
        ...
        private string GetInputPath(int fileNum)
        {
            return Path.Combine(projDir, "sample_input", $"input{fileNum}.txt");
        }
    }
}
